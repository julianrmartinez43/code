...
class Paypal extends Client {
  ...
  checkTxNonce(tx) {
    ...
    if (tx.contents.nonce > this.state[tx.contents.from].nonce && (!(tx in this.pendingTx))) {
      {
      // add the transaction to the pendingTx poo
    ...
    if (tx.contents.nonce === this.state[tx.contents.from].nonce) {
      // return true to signal that payit is ok to proceed to the nex operation
      return true
    } 
  ...
  checkTxType(tx) {
    ...
    if (tx.contents.type === 'cancel') {
      ...
      let cancellation_nonce = tx.contents.nonce; 
      // check pending transactions
      for (const i in this.pendingTx) {
        
        const pendingTx = this.pendingTx[i]
        if (pendingTx.contents.nonce === cancellation_nonce) {
          if(pendingTx.contents.from === tx.contents.from) {
